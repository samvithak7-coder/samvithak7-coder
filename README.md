...
`Arduino Uno` `C` `Sensors` `Embedded Systems`
🏆 *3rd Prize – Mini Project Exhibition*

---

### 🩺 Medical RAG Pipeline *(Local & Offline)*
Fully offline RAG pipeline that answers medical queries from a local knowledge base — no cloud API, no internet required.
Semantic search over medical knowledge using ChromaDB, local LLM inference via Ollama (qwen3.5:4b).
Delivers structured assessments: condition, severity, and recommended action.

`Python` `Ollama` `ChromaDB` `RAG` `Local LLM`
